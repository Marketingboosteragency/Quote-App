...
QUOTES_FOLDER = os.path.join(STATIC_FOLDER, 'cotizaciones')
...
# --- Clase para Extracción de Datos Web ---
            ...
            quoteForm.addEventListener('submit', async function(e) {
                ...
                try {
                    ...
                    if (result.status === 'success') {
                        const message = `PDF generated successfully. <a href="${result.file_url}" target="_blank" class="btn btn-secondary">View PDF</a>`;
                        showResult(pdfResults, message, false);
                    } else {
                        showResult(pdfResults, 'Error generating PDF: ' + result.error, true);
                    }
                } catch(err) {
                    pdfLoader.style.display = 'none';
                    showResult(pdfResults, 'Connection error: ' + err, true);
                }
            });
...
def generate_quote_route():
    try:
        ...
        file_url = url_for('static', filename=f'cotizaciones/{pdf_filename}', _external=True)
...
@app.route('/static/cotizaciones/<filename>')
